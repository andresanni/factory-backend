...
describe("RoleService", () => {
  ...
  describe("create", () => {
    ...
    it("should throw an error if the user is not authorized", async () => {
      ...
      try{
       ...
       expect((error as ServiceLayerError).internalMessage).toBe("Authorization error\n create:role isn't in user permissions. User permissions: empty");
    ...
    it("should throw an error if the role already exists", async () => {
      ...
      try{
        ...
        expect(error).toBeInstanceOf(ServiceLayerError);
        expect((error as ServiceLayerError).internalMessage).toBe("Role Test Role already exists");
      }
    });

    it("should create a new role with initial permissions", async () => {
      const roleName: string = "Test Role";
      const permissionsNames: Permissions[] = [Permissions.CREATE_ROLE, Permissions.READ_ROLE];
  ...
  describe("assignPermissionsToRole", () => {
    it("should assign existing permissions to an existing role successfully", async () => {
      ...
      const permissionsNamesToAssign: Permissions[] = [Permissions.CREATE_ROLE, Permissions.READ_ROLE];
      
      await service.createRole(authenticatedUser, roleName, [Permissions.UPDATE_ROLE]);
      const updatedRole = await service.assignPermissionsToRole(authenticatedUser, roleName, permissionsNamesToAssign);

      expect(updatedRole.permissions?.length).toBe(2);
      expect(updatedRole.permissions?.some(p => p.description === Permissions.CREATE_ROLE)).toBe(true);
      expect(updatedRole.permissions?.some(p => p.description === Permissions.READ_ROLE)).toBe(true);
    });
    ...
    it("should not throw an error when assigning existing permissions to an existing role", async () => {
      ...
      await expect(service.assignPermissionsToRole(authenticatedUser, roleName, permissionsNames)).resolves.not.toThrow();
    });
    ...
    it("should throw an error if the user does not have the UPDATE_ROLE permission", async () => {
      ...
      authenticatedUser.permissions = authenticatedUser.permissions.filter(permission => permission !== Permissions.UPDATE_ROLE);  
      await service.createRole(authenticatedUser, roleName);
      
      try{
        await service.assignPermissionsToRole(authenticatedUser, roleName, permissionsNames);
      }
      catch(error){
        expect(error).toBeInstanceOf(ServiceLayerError);
