...
export class RoleService {
  ...
  constructor(roleRepository: RoleRepository, permissionRepository: PermissionRepository) {
    this.roleRepository = roleRepository;
    this.permissionRepository = permissionRepository;
  }

  //Utility method to check if the authenticated user has the required permission
  private checkAuthorization(authenticatedUser: AuthenticatedUser, permission: Permissions, statusCode: {value: number} ): boolean {
    const authorized: boolean = authenticatedUser.permissions.includes(permission);
    if (!authorized) {
  ...
  private async resolvePermissions(permissionsNames: Permissions[], statusCode: {value: number}): Promise<Permission[]>{
    const permissionsPromises = permissionsNames.map(
  ...
  async createRole(
    ...
    try {      
      ...
      this.checkAuthorization(authenticatedUser, Permissions.CREATE_ROLE, statusCode);
      ...
      if (permissionsNames) {        
        const resolvedPermissions = await this.resolvePermissions(permissionsNames, statusCode);        
   ...
   async assignPermissionsToRole(
    ...
    const statusCode: {value: number} = {value: 500};
    try {
      this.checkAuthorization(authenticatedUser, Permissions.UPDATE_ROLE, statusCode);
